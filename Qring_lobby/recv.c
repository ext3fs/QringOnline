...
static	volatile	int	R_dbagent_fd[999];
...
void *R_Recv_main(void *arg)
{
	...
	while( G_Run )
	{
		...
		for( i=0; i<nready; i++ )
		{
			...
			//check command 
			if( ((header.command) & 0xff000000 ) != _CL )
			{
...
static	void	R_proc_TW_LIVE(void)
{
	...
	n	= snprintf(R_send_buf, 8192, "< Lobby (%d) :: G_Login_number(%d) G_Connection_number(%d)",
...
static	void	R_proc_TW_QUEUE(void)
{
	...
	n	= snprintf(R_send_buf, 8192, "< Lobby (%d) :: recv_q(%d) send_q(%d) >",
...
static	void	R_accept_fd(const int accept_fd, const void *sin_addr)
{
	...
	//check server full
	if( G_Connection_number > (MAX_FD - CONNECTION_BILLING - D_GetMax_world() - 50) )
	{
		U_Printf("[RECV]Server full : G_Connection_number(%d)\n", G_Connection_number);
		n	= MK_GetPacket_LC_SERVER_TROUBLE(R_send_buf, SERVER_TROUBLE_FULL);
...
static	void	R_recv_login_dbagent(const int login_dbagent_fd)
{
	...
	if( R_recv_fd[login_dbagent_fd].connecting )
	{
		...
		{
			U_Printf("[RECV]Error login_dbagent fd(%d), idx(%d), getsockopt() : %s\n",
	...
	{	
		if( n )
			U_Printf("[RECV]Error login_dbagent fd(%d), idx(%d), recv() : %s\n",
					login_dbagent_fd, R_recv_fd[login_dbagent_fd].login_dbagent_idx, strerror(errno));
		else
	...
	{	
		...
		{
			U_Printf("[RECV]Error login_dbagent fd(%d), idx(%d), bad header.body_size(%d)\n",
		...
		{
			U_Printf("[RECV]Error login_dbagent fd(%d), idx(%d), bad header.command(%x)\n",
...
static	int	R_connect_dbagent(const int dbagent_idx)
{
	...
	if( inet_pton(AF_INET, D_GetWorld_machine_dbagent_ip(dbagent_idx, 1), &(addr.sin_addr)) <= 0 )
	{
		U_Printf("[RECV]Error inet_pton(%s)\n", D_GetWorld_machine_dbagent_ip(dbagent_idx, 1));
    		close(sock_fd);
    		return	(-1);
	}
	...
	R_recv_fd[sock_fd].epoll_add	= 1;
	R_recv_fd[sock_fd].recv_sec	= G_Sec;
	strncpy(R_recv_fd[sock_fd].ip, D_GetWorld_machine_dbagent_ip(dbagent_idx, 1), SIZEOF_IP+1);
