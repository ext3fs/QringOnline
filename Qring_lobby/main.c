...
int	main(int argc, char **argv)
{
	...
	if( G_Demon  )
	{
		...
		U_Printf("\t[MAIN]Lobby %d Start !! ( Demon mode ) \n", G_Server_code);
	}
	...
	U_Printf("\t[MAIN]Lobby %d Shutdown\n", G_Server_code);
	return	(0);
}
...
static	int	M_ini_load(const char *file_name)
{
	...
	while( fgets(line, 8192, fp) )
	{
		...
		*ptr = 0;
		...
		if( strcmp("lobby", line) == 0 )
		{
			G_Lobby = strtoul(ptr+1, (char **)NULL, 10);
			continue;
		}
	...
	if( (G_Lobby < 1) || (G_Lobby >= 999) )
	{
		U_Printf("[MAIN]Error bad G_Lobby(%d)\n", G_Lobby);
		return	(-1);
	}
