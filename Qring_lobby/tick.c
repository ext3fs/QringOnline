...
{
	int	i, max_world, billing_idx, login_dbagent_idx, dbagent_idx, fd;
	time_t	send_tick;


	U_Printf("\t[TICK]Tick Thread Start !\n");
		
	max_world		= D_GetMax_world();
	billing_idx		= 1;
	login_dbagent_idx	= 1;
	dbagent_idx		= 1;
	fd			= 1;
	...
	while( 1 )
	{
		...
		dbagent_idx	= ( (dbagent_idx == max_world) ? 1 : dbagent_idx+1);
