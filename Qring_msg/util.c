...
void	U_Recv_queue_in(const unsigned short fd, const unsigned short fd_or_seq, const int command, const int sec_active, const int session,
			const unsigned short data_size, const char *data, const unsigned short recv_cnt)
{
	int	temp_tail;
...
void	U_Send_queue_in(const int send_type, const unsigned short id, const unsigned short map, const unsigned short block_x,
			const unsigned short data_size, const char *data)
{
	int	temp_tail;
...
static	int	U_log(const char *buf)
{
	...
	snprintf(temp_file_name, 128, "./err_log/login%03d%03d_%s.log", G_Game, G_World, temp_date_buf);
