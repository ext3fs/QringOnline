...
static int			A_msg_fd;
static char			A_msg_ipaddress[SIZEOF_IP];
...
static inline void	A_proc_tcp_fd( const int accept_fd )
{
	...
	strcpy( A_msg_ipaddress, inet_ntoa(A_acpt_addr.sin_addr) );
...
static inline int	A_proc_acpt_fd( void )
{
	...
	memcpy( send_data + 8	, &A_msg_ipaddress	, SIZEOF_IP );
	...
	for( i=0; i<MAX_RECVT_CNT; i++ )
	{
		...
		if( (n = send(G_inner_recv_fd[i][0], send_data, 8 + SIZEOF_IP, 0)) != (8 + SIZEOF_IP) )
		{
